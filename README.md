...
| `waving` | `stretch` | 완전히 늘어난 구간 안에서 원본 자세를 중복해 미세 동작만 표현 |
| `jumping` | `stretch` | 완전히 늘어난 구간 안에서 원본 자세를 중복해 미세 동작만 표현 |
...
Codex 앱은 상태별 프레임 시간과 비-idle 상태의 3회 반복을 고정합니다. 이 펫은 배포 호환성을 유지하기 위해 런타임을 수정하지 않고 원본 프레임 중복으로 누운 `sleep`, 앉은 `idle`, `roll`의 포즈 변경 빈도를 낮춥니다. `stretch`는 완전히 늘어난 구간의 미세 동작만 사용해 반복 폭을 줄인 뒤 `sleep`으로 전환합니다.
